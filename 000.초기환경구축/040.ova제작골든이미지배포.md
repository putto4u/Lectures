...
```yaml
---
- name: VirtualBox 버전 동기화 및 골든 이미지(OVA) 통합 배포
  hosts: nodes
  gather_facts: no
  vars:
    # [1. 리소스 경로 및 파일명]
    node_res_storage: "C:\\VM_Storage"
    vbox_installer: "VirtualBox-7.0.12-Win.exe"
    vbox_version: "7.0.12"
    ova_file: "ubuntu_2204_gold.ova"
    
    # [2. 실행 경로]
    vbox_path: "C:\\Program Files\\Oracle\\VirtualBox\\VBoxManage.exe"
    local_ova_source: "./resources/ubuntu_2204_gold.ova"
    local_vbox_source: "./resources/VirtualBox-7.0.12-Win.exe"
  ...
  tasks:
    ...
    # [STEP 2] 설치 파일 및 OVA 전송
    - name: 마스터 서버 리소스 전송 (VBox 설치파일 & OVA)
      win_copy:
        src: "{{ item.src }}"
        dest: "{{ node_res_storage }}\\{{ item.dest }}"
        force: no
      loop:
        - { src: "{{ local_vbox_source }}", dest: "{{ vbox_installer }}" }
        - { src: "{{ local_ova_source }}", dest: "{{ ova_file }}" }

    # [STEP 3] VirtualBox 버전 체크
    - name: 기존 VirtualBox 설치 버전 확인
      win_shell: |
        if (Test-Path "{{ vbox_path }}") {
          $version = & "{{ vbox_path }}" --version
          return $version
        } else {
          return "None"
        }
      register: installed_vbox

    # [STEP 4] 버전 불일치 시 강제 설치 (업데이트)
    - name: 권장 버전({{ vbox_version }})으로 VirtualBox 설치/업데이트
      win_package:
        path: "{{ node_res_storage }}\\{{ vbox_installer }}"
        arguments: /s
        state: present
      when: vbox_version not in installed_vbox.stdout
      register: vbox_install_res

    # [STEP 5] 네트워크 드라이버 갱신에 따른 WinRM 재연결 대기
    - name: 가상 네트워크 어댑터 설정 및 연결 안정화 대기 (60초)
      pause:
        seconds: 60
      when: vbox_install_res.changed

    # [STEP 6] 골든 이미지(OVA) 자동 임포트
    - name: VirtualBox에 골든 이미지 자동 임포트
      win_shell: |
        & "{{ vbox_path }}" import "{{ node_res_storage }}\\{{ ova_file }}" --vsys 0 --vmname "{{ vm_name }}"
    ...
    - name: 배포 및 구성 결과 확인
      debug:
        msg: "노드 PC의 VirtualBox 버전이 {{ vbox_version }}으로 동기화되었으며, {{ vm_name }} 임포트가 완료되었습니다."
