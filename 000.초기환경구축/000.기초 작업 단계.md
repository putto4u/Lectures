...
sudo visudo   # sudo 패스워드 안넣기,
   ubuntu ALL=(ALL) NOPASSWD: ALL  #  맨 마지막 줄에 추가하기

# 머신 아이디 파일 비우기
sudo truncate -s 0 /etc/machine-id
# 리포짓토리에 연결된 복사본이 있다면 삭제 (시스템에 따라 다름)
...
**재접속시 안되면**
  sudo hostnamectl set-hostname --static vmmaster01
  hostnamectl status
  ```
